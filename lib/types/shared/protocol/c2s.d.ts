...
export type ClientToServer = {
    ...
    type: 'c2s.window.open';
    payload: {
        appId: string;
        hint?: string;
    };
} | {
    type: 'c2s.window.close';
    payload: {
        windowId: string;
    };
} | {
    type: 'c2s.window.focus';
    payload: {
        windowId: string;
    };
} | {
    type: 'c2s.window.minimize';
    payload: {
        windowId: string;
    };
} | {
    type: 'c2s.window.maximize';
    payload: {
        windowId: string;
    };
} | {
...
/** Single-step undo/redo of a window's last AI content change. */
 | {
    type: 'c2s.window.undo';
    payload: {
        windowId: string;
    };
} | {
    type: 'c2s.window.redo';
    payload: {
        windowId: string;
    };
} | {
...
/** Set the desktop wallpaper from an uploaded image (a data: URL). */
 | {
    type: 'c2s.wallpaper.upload';
    payload: {
        dataUrl: string;
    };
} | {
    type: 'c2s.notification.read';
    payload: {
        id: string | 'all';
    };
} | {
    type: 'c2s.notification.click';
    payload: {
        id: string;
    };
}
...
/** Ask for the resolved role models + the DSH provider/model catalog. */
 | {
    type: 'c2s.models.list';
    payload: Record<string, never>;
}
/** Factory-reset the desktop session (settings are kept). */
 | {
    type: 'c2s.system.reset';
    payload: Record<string, never>;
} | {
    type: 'c2s.session.list';
    payload: Record<string, never>;
} | {
    type: 'c2s.session.restore';
    payload: {
        id: string;
    };
} | {
    type: 'c2s.session.export';
    payload: {
        id: string;
    };
} | {
    type: 'c2s.session.import';
    payload: {
        json: string;
    };
};
export type ClientToServerType = ClientToServer['type'];
export type ClientToServerPayload<T extends ClientToServerType> = Extract<ClientToServer, {
    type: T;
}>['payload'];
