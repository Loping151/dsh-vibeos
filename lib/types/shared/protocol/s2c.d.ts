...
export type WireErrorCode = 'bad_json' | 'bad_message' | 'internal' | 'no_app' | 'ai_failed' | 'wallpaper_bad_image';
export type ServerToClient = {
    type: 's2c.boot.state';
    payload: BootStatePayload;
} | {
    type: 's2c.boot.ready';
    payload: Record<string, never>;
} | {
    type: 's2c.ui.patch';
    payload: UiPatchPayload;
} | {
    type: 's2c.ui.busy';
    payload: {
        windowId: string;
    ...
    type: 's2c.window.opened';
    payload: {
        window: WindowState;
    };
} | {
    type: 's2c.window.closed';
    payload: {
        windowId: string;
    };
} | {
    type: 's2c.window.focused';
    payload: {
        windowId: string;
    };
} | {
    type: 's2c.window.moved';
    payload: {
        window: WindowState;
    };
} | {
    type: 's2c.window.stateChanged';
    payload: {
        window: WindowState;
    };
} | {
    type: 's2c.window.reordered';
    payload: {
        ids: string[];
    };
} | {
    type: 's2c.syscall.notify';
    payload: {
    ...
    type: 's2c.syscall.fileCreated';
    payload: {
        node: VfsNode;
    };
} | {
    type: 's2c.vfs.changed';
    payload: {
        node: VfsNode;
    };
} | {
    type: 's2c.vfs.removed';
    payload: {
        ids: string[];
    };
}
...
/** The desktop session was factory-reset; clients drop state and re-hello. */
 | {
    type: 's2c.system.reset';
    payload: Record<string, never>;
} | {
    type: 's2c.session.exported';
    payload: {
        name: string;
        json: string;
    };
...
export type ServerToClientPayload<T extends ServerToClientType> = Extract<ServerToClient, {
    type: T;
}>['payload'];
