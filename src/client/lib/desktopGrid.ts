/* Derived from VibeOS (https://github.com/benis-me/VibeOS) — apps/frontend/src/lib/desktopGrid.ts
 * (+ the autoArrange loop from components/desktop/Desktop.tsx). Adapted for DeepSeek Harness
 * (dsh-vibeos). Original license: MIT. */
...
export function autoArrange(
  ...
  return nodes.map((n, i) => {
    const { x, y } = gridPosition(i, viewportH);
