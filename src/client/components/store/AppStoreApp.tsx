/* Derived from VibeOS (https://github.com/benis-me/VibeOS) — apps/frontend/src/components/store/AppStoreApp.tsx.
 * Adapted for DeepSeek Harness (dsh-vibeos): inline icon set. Original license: MIT. */
...
import { Download, Trash2, Upload } from '../../icons/uiIcons';
  ...
  return (
    <div className="h-full overflow-auto bg-background text-foreground">
      <div className="mx-auto max-w-3xl p-6">
          ...
          <div className={GRID}>
            {installed.map((a) => (
              <AppCard
                ...
                onUninstall={() => wsClient.send('c2s.app.uninstall', { appId: a.id })}
                t={t}
              />
            ))}
          </div>
        ...
        <label className="vibe-btn mb-2 inline-flex cursor-pointer items-center gap-1.5 rounded-lg border bg-card px-2.5 py-1.5 text-[12px] transition-colors hover:bg-accent">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              e.target.value = '';
              if (!f) return;
              void f.text().then((txt) => setImportText(txt));
            }}
          />
        </label>
        <textarea
      ...
      <div className="mt-auto flex gap-1.5">
        ...
        {onUninstall && (
          <button
            ...
            className={
              'vibe-btn rounded-md border px-2 py-1 transition-colors ' +
              (confirming
