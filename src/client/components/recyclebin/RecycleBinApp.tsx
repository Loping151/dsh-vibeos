/* Derived from VibeOS (https://github.com/benis-me/VibeOS) — apps/frontend/src/components/recyclebin/RecycleBinApp.tsx.
 * Adapted for DeepSeek Harness (dsh-vibeos): inline icon set. Original license: MIT. */

import { useMemo } from 'react';
import type { AppDescriptor, VfsNode } from '../../../shared/index';
...
import { RotateCw, Trash2 } from '../../icons/uiIcons';

type T = (k: string) => string;

const FolderIcon = NAME_ICONS['folder']!;
const FileIcon = NAME_ICONS['file-text']!;
...
function Row({
  ...
  const icon =
    node.type === 'shortcut' ? (
      <AppIcon
        name={app?.icon ?? (node.meta.icon as string | undefined)}
  ...
  return (
    <div className="flex items-center gap-3 border-b border-border/50 px-5 py-2.5 transition-colors hover:bg-accent/30">
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        >
          <RotateCw className="size-3.5" /> {t('recyclebin.restore')}
        </button>
        <button
          onClick={onDelete}
