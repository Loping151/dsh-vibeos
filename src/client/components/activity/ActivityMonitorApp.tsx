/* Derived from VibeOS (https://github.com/benis-me/VibeOS) — apps/frontend/src/components/activity/ActivityMonitorApp.tsx.
 * Adapted for DeepSeek Harness (dsh-vibeos): the cost metric and column are dropped (DSH routes carry no
 * price data) and icons come from the inline set. Original license: MIT. */
...
import { Activity, Loader2, StopCircle } from '../../icons/uiIcons';
...
const COLS =
  'grid grid-cols-[minmax(104px,1.2fr)_minmax(130px,1.8fr)_minmax(80px,0.9fr)_66px_50px_58px_60px] gap-3';
...
export function ActivityMonitorApp() {
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      cacheRead,
      reasoning,
      // Cache hit rate is against billed input (which already folds cache reads in).
      cacheRate: input ? cacheRead / input : 0,
      running,
    ...
    <div className="flex h-full flex-col bg-background text-foreground">
      <div className="shrink-0 border-b">
        ...
        <div className="flex flex-wrap gap-x-7 gap-y-2 px-5 py-3">
          ...
          <Metric
            label={t('activity.inOut')}
            value={`${fmtTokens(summary.input)} / ${fmtTokens(summary.output)}`}
          />
          <Metric
            label={t('activity.cacheRate')}
            value={summary.input ? `${(summary.cacheRate * 100).toFixed(0)}%` : '—'}
            hint={summary.cacheRead ? fmtTokens(summary.cacheRead) : undefined}
          />
          {summary.reasoning > 0 && (
            <Metric label={t('activity.reasoning')} value={fmtTokens(summary.reasoning)} />
          )}
          <Metric
    ...
    >
      ...
      <div className="flex justify-end">
        {r.status === 'running' ? (
          ...
          >
            <StopCircle className="size-2.5" /> {t('activity.stop')}
          </button>
        ) : (
