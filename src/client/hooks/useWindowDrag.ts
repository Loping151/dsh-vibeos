/* Derived from VibeOS (https://github.com/benis-me/VibeOS) — apps/frontend/src/hooks/useWindowDrag.ts.
 * Adapted for DeepSeek Harness (dsh-vibeos). Original license: MIT. */

import { useCallback, useRef } from 'react';
import type { Rect } from '../../shared/index';
...
export function useWindowDrag(windowId: string) {
  ...
  const begin = useCallback(
    (dir: ResizeDir | 'move', e: React.PointerEvent) => {
      ...
      useWindowStore.getState().setDragging(windowId);
      ...
      const onUp = () => {
        useWindowStore.getState().setDragging(null);
