/* Derived from VibeOS (https://github.com/benis-me/VibeOS) — apps/frontend/src/hooks/useAnchoredPopover.ts.
 * Adapted for DeepSeek Harness (dsh-vibeos): the trigger is queried inside #vibeos-root only
 * (never the whole document). Original license: MIT. */
...
export function useAnchoredPopover(
  ...
  useLayoutEffect(() => {
    if (!open) return;
    const place = () => {
      const el = getVibeosRoot()?.querySelector(triggerSelector);
      if (!el) return;
