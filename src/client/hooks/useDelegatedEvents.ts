/* Derived from VibeOS (https://github.com/benis-me/VibeOS) — apps/frontend/src/hooks/useDelegatedEvents.ts.
 * Adapted for DeepSeek Harness (dsh-vibeos). Every delegation rule is a behavioral contract —
 * do not deviate. Original license: MIT. */
...
export function useDelegatedEvents(
  ...
  useEffect(() => {
    ...
    const submitForm = (form: HTMLFormElement, action?: string) => {
      ...
      // A submitted form clears its free-text fields (terminal/chat semantics);
      // region patches rarely re-render the form. Opt out with data-vibeos-keep.
      if (!('vibeosKeep' in form.dataset)) {
        for (const f of fields) {
          if (
            f.tagName === 'TEXTAREA' ||
            (f.tagName === 'INPUT' &&
        ...
        root.dispatchEvent(
          new CustomEvent('vibeos:submitted', { bubbles: false, detail: { value: primary } }),
        );
      }
    };
    ...
    const onClick = (e: MouseEvent) => {
      ...
      const isSubmit =
        btn.type === 'submit' ||
        ((btn.tagName === 'BUTTON' || (btn as unknown as HTMLInputElement).type === 'submit') &&
          !btn.type);
      if (form && isSubmit) {
