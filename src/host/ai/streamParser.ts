/* Derived from VibeOS (https://github.com/benis-me/VibeOS) — apps/backend/src/ai/streamParser.ts.
 * Adapted for DeepSeek Harness (dsh-vibeos). Original license: MIT. */

import { syscallSchema, type ParsedAiOutput } from '../../shared/prompt/syscall-schema';
...
export function parseAiOutput(full: string): ParsedAiOutput {
  ...
  // A applet replaces the whole window body: it runs in a sandboxed iframe.
  if (applet) {
...
function parseSyscalls(full: string): ParsedAiOutput['syscalls'] {
  ...
  try {
    ...
    log.warn('unparseable block dropped');
    return [];
  }
  ...
  for (const item of raw) {
    ...
    if (parsed.success) out.push(parsed.data);
      ...
      log.warn(
        `dropped invalid call: ${parsed.error.issues[0]?.message}`,
